  ...
  try {
    ...
    const body = {
      ...
      redirectUrls: {
        ...
        cancelUrl: `${VITE_API_URL}/linepay-cancel`,
      },
    };

    const signature = generateSignature(LINEPAY_SECRET, '/v3/payments/request', body, nonce);
...
exports.linePayConfirmCallback = async (req, res) => {
  ...
  if (!transactionId || !orderId || !userId) {
    return res.redirect(`${VITE_API_URL}/schedule?linepayResult=fail`);
  }
  ...
  try {
    ...
    if (confirmRes.data.returnCode === '0000') {
      ...
      return res.redirect(`${VITE_API_URL}/schedule?linepayResult=success`);
    } else {
      return res.redirect(`${VITE_API_URL}/schedule?linepayResult=fail`);
    }
  } catch (err) {
  ...
  try {
    ...
    try {
      ...
      if (jwtError.name === 'JsonWebTokenError' || jwtError.name === 'TokenExpiredError') {
        return res
          ...
          .json({ isPremium: false, message: '無效或過期的認證權限' });
      }
      return res
        .status(HTTP.INTERNAL_SERVER_ERROR)
        .json({ isPremium: false, message: 'JWT 驗證失敗' });
    }
    ...
    if (!userId) {
      return res
        ...
        .json({ isPremium: false, message: '認證失敗：無法獲取用戶 ID' });
    }
    ...
    return res
      .status(HTTP.INTERNAL_SERVER_ERROR)
      .json({ isPremium: false, message: '檢查會員狀態失敗' });
  }
};
